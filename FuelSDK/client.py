...
class ET_Client(object):
    ...
    authTokenExpiration = (
        None  # seconds since epoch that the current jwt token will expire
    )
    refreshKey = None
    endpoint = None
    authObj = None
    ...
    def __init__(
        self, get_server_wsdl=False, debug=False, params=None, tokenResponse=None
    ):
        self.debug = debug
        if debug:
    ...
    def configure_client(self, get_server_wsdl, params, tokenResponse):
        ...
        if Path("~/.fuelsdk/config.python").exists():
            config.read(Path("~/.fuelsdk/config.python"))
        else:
        ...
        if params is not None:
            self.client_id = (
                params.get("clientid")
                or config.get("Web Services", "clientid")
                or os.environ["FUELSDK_CLIENT_ID"]
            )
            ...
            if "baseapiurl" in params:
                ...
                self.base_api_url = "https://www.marketingcloud.com"

            if "authenticationurl" in params:
            ...
            self.wsdl_file_url = self.load_wsdl(
                wsdl_server_url, wsdl_file_local_location, get_server_wsdl
            )

            if "useOAuth2Authentication" in params:
                self.use_oAuth2_authentication = params["useOAuth2Authentication"]
            elif config.has_option("Auth Service", "useOAuth2Authentication"):
                self.use_oAuth2_authentication = config.get(
                    "Auth Service", "useOAuth2Authentication"
                )
        ...
        if self.is_none_or_empty_or_blank(self.auth_url):
            if self.use_oAuth2_authentication == "True":
                raise Exception(
                    "authenticationurl (Auth TSE) is mandatory when using OAuth2 authentication"
                )
            else:
                self.auth_url = (
        ...
        if self.application_type in ["public", "web"]:
            if self.is_none_or_empty_or_blank(
                self.authorization_code
            ) or self.is_none_or_empty_or_blank(self.redirect_URI):
                raise Exception(
        ...
        if self.application_type == "public":
            ...
            if self.is_none_or_empty_or_blank(
                self.client_id
            ) or self.is_none_or_empty_or_blank(self.client_secret):
                raise Exception(
        ...
        if params is not None and "jwt" in params:
            ...
            self.authTokenExpiration = (
                time.time() + decodedJWT["request"]["user"]["expiresIn"]
            )
    ...
    def load_wsdl(self, wsdl_url, wsdl_file_local_location, get_server_wsdl=False):
        ...
        if (
            ...
            if r is not None and "last-modified" in r.headers:
                server_wsdl_updated = time.strptime(
    ...
    def build_soap_client(self):
        ...
        self.soap_client = suds.client.Client(
            self.wsdl_file_url, faults=False, cachingpolicy=1
        )
    ...
    def refresh_token(self, force_refresh=False):
        ...
        if (
            ...
            or (
                self.authTokenExpiration is not None
    ...
    def get_soap_endpoint(self):
        ...
        if (
            "url" in cache_file_data
            and "timestamp" in cache_file_data
