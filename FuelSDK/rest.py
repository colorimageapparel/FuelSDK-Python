...
class ET_Constructor(object):
    ...
    def __init__(self, response=None, rest=False):
        if response is not None:  # if a response was returned from the web service call
            if rest:  # result is from a REST web service call...
                ...
                self.status = True if self.code == 200 else False
                ...
                if self.code == 200:
                    ...
                    if (
                        ...
                        body_container_tag: str | None = "Results"
                    elif (
                        "ObjectDefinition" in body
    ...
    def parse_props_dict_into_ws_object(
        self, obj_type: str, ws_object: dict, props_dict: dict
    ) -> dict:
        for k, v in list(props_dict.items()):
            if k in ws_object:
                ws_object[k] = v
            else:
                message: str = f"{k} is not a property of {obj_type}"
    ...
    def parse_props_into_ws_object(self, auth_stub, obj_type, props):
        ...
        if props is not None and type(props) is dict:
            ...
            message = f"Can't post properties to {obj_type} without a dict or list of properties"
            raise Exception(message)
...
class ET_Describe(ET_Constructor):
    def __init__(self, auth_stub, obj_type):
        ...
        ws_describeRequest = auth_stub.soap_client.factory.create(
            "ArrayOfObjectDefinitionRequest"
        )
...
class ET_Configure(ET_Constructor):
    def __init__(self, auth_stub, obj_type, props=None, update=False, delete=False):
        ...
        ws_configureRequest = auth_stub.soap_client.factory.create(
            "ConfigureRequestMsg"
        )
        if delete:
            action: str = "delete"
...
class ET_Get(ET_Constructor):
    def __init__(
        self, auth_stub, obj_type, props=None, search_filter=None, options=None
    ):
        ...
        if (
            props is None
        ):  # if there are no properties to retrieve for the obj_type then return a Description of obj_type
            describe = ET_Describe(auth_stub, obj_type)
            props = [p.Name for p in describe.results[0].Properties if p.IsRetrievable]

            # List comprehension decompressed below:
        ...
        if props is not None:
            ws_retrieveRequest = self.make_prop_requests(ws_retrieveRequest, props)

        if search_filter is not None:
            ws_retrieveRequest.Filter = self.make_search_filter(
                search_filter, auth_stub
            )
        ...
        # 				ws_retrieveRequestRequest.Options[key][k] = v
        # 		else:
    ...
    def make_prop_requests(self, ws_retrieveRequest, props):
        """Used to reduce complexity from __init__"""
        if (
            type(props) is dict
        ...
        return ws_retrieveRequest

    def make_search_filter(self, search_filter, auth_stub):
        """Create the needed search filter"""
        if "LogicalOperator" in search_filter:
            ws_simpleFilterPartLeft = auth_stub.soap_client.factory.create(
                "SimpleFilterPart"
            )
            for prop in ws_simpleFilterPartLeft:
                # print prop[0]
                if prop[0] in search_filter["LeftOperand"]:
                    ws_simpleFilterPartLeft[prop[0]] = search_filter["LeftOperand"][
                        prop[0]
                    ]

            ws_simpleFilterPartRight = auth_stub.soap_client.factory.create(
                "SimpleFilterPart"
            )
            for prop in ws_simpleFilterPartRight:
                if prop[0] in search_filter["RightOperand"]:
                    ws_simpleFilterPartRight[prop[0]] = search_filter["RightOperand"][
                        prop[0]
                    ]

            ws_complexFilterPart = auth_stub.soap_client.factory.create(
                "ComplexFilterPart"
            )
            ...
            for additional_operand in search_filter.get("AdditionalOperands", []):
                ws_simpleFilterPart = auth_stub.soap_client.factory.create(
                    "SimpleFilterPart"
                )
                for k, v in list(additional_operand.items()):
                    ws_simpleFilterPart[k] = v
                ws_complexFilterPart.AdditionalOperands.Operand.append(
                    ws_simpleFilterPart
                )

            return ws_complexFilterPart
        else:
            ws_simpleFilterPart = auth_stub.soap_client.factory.create(
                "SimpleFilterPart"
            )
            ...
            return ws_simpleFilterPart

    def __str__(self):
        return f"{{auth_stub: {self.auth_stub}, options: {self.options}}}"
...
class ET_Patch(ET_Constructor):
    def __init__(self, auth_stub, obj_type, props=None):
        auth_stub.refresh_token()
        response = auth_stub.soap_client.service.Update(
            None, self.parse_props_into_ws_object(auth_stub, obj_type, props)
        )
...
class ET_Delete(ET_Constructor):
    def __init__(self, auth_stub, obj_type, props=None):
        auth_stub.refresh_token()
        response = auth_stub.soap_client.service.Delete(
            None, self.parse_props_into_ws_object(auth_stub, obj_type, props)
        )
...
class ET_GetRest(ET_Constructor):
    def __init__(self, auth_stub, endpoint, qs=None):
        ...
        headers = {
            "authorization": "Bearer " + auth_stub.authToken,
            "user-agent": "FuelSDK-Python-DI",
        }
