...
class ET_ProfileAttribute:
    ...
    def post(self):
        obj = ET_Configure(
            self.auth_stub, self.obj_type, self.props, self.update, self.delete
        )
        if obj is not None:
...
class ET_Campaign(ET_CUDSupportRest):
    ...
    def patch(self):
        self.path = (
            "/hub/v1/campaigns"  # don't put the id on the url when patching via post
        )
...
class ET_Interactions(ET_CUDSupportRest):
    def __init__(self):
        super(ET_Interactions, self).__init__()
        self.path = "/interaction/v1/interactions"
        self.urlProps = []
        self.urlPropsRequired = []
...
class ET_LinkSend(ET_CUDSupport):
    def __init__(self):
...
class ET_Link(ET_CUDSupport):
    def __init__(self):
        super(ET_Link, self).__init__()
        self.obj_type = "Link"
...
class ET_DataExtension_Row(ET_CUDSupport):
    ...
    def get(self):
        ...
        obj = ET_Get(
            self.auth_stub,
            "DataExtensionObject[{0}]".format(self.Name),
            self.props,
            self.search_filter,
        )
        self.last_request_id = obj.request_id
    ...
    def getCustomerKey(self):
        if self.CustomerKey is None:
            if self.Name is None:
                ...
                if (
                    getResponse.status
                    and len(getResponse.results) == 1
                    and "CustomerKey" in getResponse.results[0]
                ):
        ...
        if self.Name is None:
            if self.CustomerKey is None:
                ...
                if (
                    getResponse.status
                    and len(getResponse.results) == 1
                    and "Name" in getResponse.results[0]
                ):
